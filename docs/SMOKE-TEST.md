...
| 步骤 | 操作 | 期望 |
|------|------|------|
| 3.1 | 未登录状态下点击「保存到 Supabase」 | 按钮禁用或提示需登录 |
| 3.2 | （可选）用 REST 直接 POST 无 JWT | 403 / RLS 拒绝 |

## 4. 登录上传

| 步骤 | 操作 | 期望 |
|------|------|------|
| 4.1 | 点击登录，输入邮箱 | 收到 Magic Link |
| 4.2 | 完成登录 | 顶栏显示「退出」 |
| 4.3 | 选择 PNG 上传 | 画布热更新预览 |
| 4.4 | 保存 | 成功 toast；Storage 有新文件；DB version +1 |
